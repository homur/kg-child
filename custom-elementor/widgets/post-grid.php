 ...
 * @since 1.1.0
 */
class Cstm_Post_Grid_Widget extends Widget_Base
{

	public function get_name()
	{
		return 'Cstm-blog-posts';
	}

	public function get_title()
	{
		return __('Cstm Post Grid Widget', 'post-grid-elementor-addon');
	}
...
?>
		...
		<div class="wpcap-grid">
			...
			$arrLength = count($settings['post_categories']);
			...
			?>
			<div class="wpcap-grid-container elementor-grid custom-post-grid-container <?php echo $columns_desktop . $columns_tablet . $columns_mobile . $grid_class; ?>">
				...
				<nav>
					<div class="nav nav-tabs" id="nav-tab" role="tablist">
						...
						for ($i = 0; $i < $arrLength; $i++) { ?>
							...
							<a class="nav-item <?= $i === 0 ? 'active show' : '' ?>" id="nav-header-<?php echo $i; ?>" data-toggle="tab" href="#nav-<?php echo $i; ?>" role="tab" aria-controls="nav-<?php echo $i; ?>" aria-selected="true">
								<?php echo $settings['post_categories'][$i] ?>
							</a>
						<?php
						} ?>
					</div>
				...
				<div class="tab-content" id="nav-tabContent">
					...
					for ($i = 0; $i < $arrLength; $i++) {
						...
						$query_args = array(
							...
							'post_status'         	=> 'publish',
							'ignore_sticky_posts'   => true,
							'category_name' 		=> $settings['post_categories'][$i]
						);
					...
					?>
							<!-- <div style="width: 500px; height: 250px; background-color: red"></div> -->
							<div class="tab-pane fade <?= $i === 0 ? 'active show' : '' ?>" id="nav-<?php echo $i; ?>" role="tabpanel" aria-labelledby="nav-<?php echo $i; ?>">
								<?php if ($settings['post_categories'][$i] == 0) {
									include(__DIR__ . '/layouts/layout-1.php');
								} elseif ($settings['post_categories'][$i] == 1) {
									include(__DIR__ . '/layouts/layout-1.php');
								} ?>
							</div>
					<?php
	...
	protected function render_title()
	{
		...
		?>
		<<?php echo $title_tag; ?> class="title <?php if (get_the_post_thumbnail_url($this->ID, 'post-thumbnail')) { echo "";} else {echo "custom-title-style-none-image";}?>">
			<a href="<?php the_permalink(); ?>"><?php the_title(); ?></a>
		</<?php echo $title_tag; ?>>
	<?php
	}

	protected function render_post_link()
	{
	...
	?>
		<a class="read-more-btn" href="<?php the_permalink(); ?>">View Post</a>
<?php
